...
function buy(item: StoreItem) {
  const ok = finance.debit(item.price, item.name, 'clothing', simTime.day)
...
<template>
  <SceneShell title="Clothing Store">
    ...
    <ul class="item-list">
      <li v-for="item in clothingStoreItems" :key="item.id" class="item-row">
        <div class="item-info">
...
.buy-btn:hover { background: #9c4221; }
